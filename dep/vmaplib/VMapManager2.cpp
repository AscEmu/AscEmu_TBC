	...
	*/
	...
	WorldModel* VMapManager2::acquireModelInstance(const std::string & basepath, const std::string & filename)
	{
		...
		if(model == iLoadedModelFiles.end())
		{
			...
			{
				Log.outError("VMapManager2","Could not load '%s%s.vmo'!", basepath.c_str(), filename.c_str());
				delete worldmodel;
				return NULL;
			}
			Log.outError("VMapManager2","Loading file '%s%s'.", basepath.c_str(), filename.c_str());
	...
	void VMapManager2::releaseModelInstance(const std::string & filename)
	{
		...
		if(model == iLoadedModelFiles.end())
		{
			Log.outError("VMapManager2","Trying to unload non-loaded file '%s'!", filename.c_str());
			return;
		}
		if(model->second.decRefCount() == 0)
		{
			Log.outError("VMapManager2","Unloading file '%s'", filename.c_str());
