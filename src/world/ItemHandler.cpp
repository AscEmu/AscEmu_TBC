...
void WorldSession::HandleSwapInvItemOpcode( WorldPacket & recv_data )
{
  ...
  if( _player->isDead() ) {
	...
	if (srcslot > 14 && srcslot < 18)
	{
		...
		if (SrcItem != 0)
		{
			_player->RestoreItemVisual(srcslot, SrcItem);
			sEventMgr.AddEvent<Player, int8, int8>
...
void WorldSession::HandleSellItemOpcode( WorldPacket & recv_data )
{
	...
	if (quantity == 0)
	{ // Dword: quantity should not be zero (client expects that the sell prices is > 0 )
		quantity = 1;
	}
...
//	int8 bagslot = 0;
	...
	if( amount > 1 && amount > it->MaxCount )
	{ // Dword: it->MaxCount gives the maximum amount of instances of the item a player can have
		_player->GetItemInterface()->BuildInventoryChangeError(0, 0, INV_ERR_CANT_CARRY_MORE_OF_THIS);
		return;
	}
...
ARCEMU_INLINE bool RepairItem(Player * pPlayer, Item * pItem)
{
	...
	for (; i < EQUIPMENT_SLOT_END; i++)
	{
		...
		if (curItem && curItem->GetGUID() == pItem->GetGUID())
			break;
	}
