...
uint32 QuestMgr::CalcStatus(Object* quest_giver, Player* plr)
{
	...
	else if( quest_giver->GetTypeId() == TYPEID_UNIT )
	{
		...
		if(bValid)
		{
			Creature* pCreature = static_cast<Creature*>(quest_giver);
			if (pCreature->isTrainer())
			{
				if (pCreature->GetTrainer()->RequiredClass != plr->getClass())
				{ // Dword: don't enable quests from trainers to players who are not the designated class
					return QMGR_QUEST_NOT_AVAILABLE;
				}
			}
