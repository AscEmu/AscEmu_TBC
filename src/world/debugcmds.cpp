...
// Dword: added this to get update values dynamically
bool ChatHandler::HandleGetUint32ValueCommand(const char* args, WorldSession* m_session)
{
	Player* player = m_session->GetPlayer();
	if (player == 0)
	{
		sLog.outError("Error in ChatHandler::GetUint32ValueCommand : player not found.");
		return true;
	}

	if (strlen(args) == 0)
	{
		RedSystemMessage(m_session, "Invalid paramater.");
		return true;
	}
	
	uint32 index = 0;
	int result = sscanf(args, "%u", &index);
	if (result == 0)
	{
		RedSystemMessage(m_session, "Invalid paramater.");
		return true;
	...
	uint64 selectedGUID = player->GetSelection();
	Unit* u_selected = player->GetMapMgr()->GetUnit(selectedGUID);
	if (u_selected == 0)
	{
		RedSystemMessage(m_session, "You need to select a creature or character.");
		return true;
	}

	switch (u_selected->GetTypeId())
	{
	case TYPEID_PLAYER:
		{
			if (index >= PLAYER_END)
			{
				...
				sStream << "index must be less than " << PLAYER_END << '.';
				RedSystemMessage(m_session, sStream.str().c_str());
				return true;
			}
		} break;
	case TYPEID_UNIT:
		{
			...
			{
				std::stringstream sStream;
				sStream << "index must be less than " << UNIT_END << '.';
				RedSystemMessage(m_session, sStream.str().c_str());
				return true;
			}
		} break;
	...
	uint32 value = u_selected->GetUInt32Value(index);
	std::stringstream sStream;
	sStream << "Value at index " << index << ": " << value << '.';
	BlueSystemMessage(m_session, sStream.str().c_str());
