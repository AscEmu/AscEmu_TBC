...
void Player::_EventExploration()
{
    ...
    {
        // check faction
        if ((at->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 0) || (at->team == AREAC_HORDE_TERRITORY && GetTeam() == 1))
        {
            rest_on = true;
        }
    ...
    else
    {
        ...
        if (at->zone)
        {
            ...
            {
                if ((at2->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 0) || (at2->team == AREAC_HORDE_TERRITORY && GetTeam() == 1))
                {
                    rest_on = true;
                }
            ...
            for (;;)
            {
                ...
                *end = 0;
                ...
                if (v1 & 0xffff)
                {
                    ...
                    m_skills.insert(std::make_pair(sk.Skill->id, sk));
                }
            }
        }
        else
...
void Player::ResurrectPlayer()
{
    ...
    if (m_resurrectHealth)
        SetUInt32Value(UNIT_FIELD_HEALTH, (uint32)std::min(m_resurrectHealth, m_uint32Values[UNIT_FIELD_MAXHEALTH]));
    if (m_resurrectMana)
        SetUInt32Value(UNIT_FIELD_POWER1, (uint32)std::min(m_resurrectMana, m_uint32Values[UNIT_FIELD_MAXPOWER1]));
...
float Player::GetDodgeChance()
{
    ...
    return std::max(chance, 0.0f); // make sure we dont have a negative chance
}
...
void Player::UpdatePvPArea()
{
    ...
    else
    {
        ...
        if (at->flags & AREA_CITY_AREA || at->flags & AREA_CITY)
        {
            if ((at->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 1) || (at->team == AREAC_HORDE_TERRITORY && GetTeam() == 0))
            {
        ...
        {
            auto at2 = MapManagement::AreaManagement::AreaStorage::GetAreaById(at->zone);
            if (at2 && (at2->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 0) || at2 && (at2->team == AREAC_HORDE_TERRITORY && GetTeam() == 1))
            {
                if (!HasFlag(PLAYER_FLAGS, PLAYER_FLAG_PVP_TOGGLE) && !m_pvpTimer)
                {
                    // I'm flagged and I just walked into a zone of my type. Start the 5min counter.
            ...
            if (at2 && at2->flags & AREA_CITY_AREA || at2 && at2->flags & AREA_CITY)
            {
                if (at2 && (at2->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 1) || at2 && (at2->team == AREAC_HORDE_TERRITORY && GetTeam() == 0))
                {
                    if (!IsPvPFlagged()) SetPvPFlag();
                    StopPvPTimer();
                    return;
...
void Player::LoginPvPSetup()
{
    ...
    if (at != NULL && isAlive() && (at->team == AREAC_CONTESTED || (GetTeam() == 0 && at->team == AREAC_HORDE_TERRITORY) || (GetTeam() == 1 && at->team == AREAC_ALLIANCE_TERRITORY)))
...
void Player::PvPToggle()
{
    if (sWorld.GetRealmType() == REALM_PVE)
    {
        ...
        else
        {
            ...
            {
                auto at = this->GetArea();
                if (at && at->flags & AREA_CITY_AREA || at && at->flags & AREA_CITY)
                {
                    if (at && (at->team == AREAC_ALLIANCE_TERRITORY && GetTeam() == 1) || at && (at->team == AREAC_HORDE_TERRITORY && GetTeam() == 0))
                    {
                    }
                    else
                    {
...
void Player::OnWorldPortAck()
{
    ...
    if (pMapinfo)
    {
        ...
        if (pMapinfo->HasFlag(WMI_INSTANCE_WELCOME) && GetMapMgr())
        {
            ...
            welcome_msg = std::string(GetSession()->LocalizedWorldSrv(62)) + " ";
...
void Player::Possess(Unit * pTarget)
{
    ...
    if (!(pTarget->m_isPet && static_cast<Pet*>(pTarget) == m_Summon))
    {
        ...
        for (std::list<AI_Spell*>::iterator itr = pTarget->GetAIInterface()->m_spells.begin(); itr != pTarget->GetAIInterface()->m_spells.end(); ++itr)
        {
...
void Player::Social_AddFriend(const char * name, const char * note)
{
    ...
    if (note != NULL)
        ...
        m_friends.insert(std::make_pair(info->guid, (char*)NULL));
